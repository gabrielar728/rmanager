<table class="table table-striped" id="table-group-info">
    ...
    <tr>
        ...
        <th style="padding-left: 10px; width: 60%;">Denumire</th>
    ...
    @foreach($groups as $key => $group)
        <tr>
            ...
            <td style="padding-left: 10px; padding-top: 8px !important; width: 60%;">{{ $group->name }}</td>
            ...
            <td style="padding-left: 10px; padding-top: 8px !important;">
                @if($group->status == 1)
                    <a class="btn btn-success btn-sm" href="{{ route('changeGroupActiveStatus', $group->id) }}">Activ</a>
                @else
                    <a class="btn btn-danger btn-sm" href="{{ route('changeGroupInactiveStatus', $group->id) }}">Inactiv</a>
                @endif
                <button class="btn btn-primary btn-sm sorting" id="{{ $group->id }}">Ordonare</button>
            </td>
        </tr>
        <tr style="display: none;" id="group_sort{{ $group->id }}">
            <td style="" class="hide"></td>
            <td style="" class="hide"></td>
